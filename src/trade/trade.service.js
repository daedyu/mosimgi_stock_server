...
exports.buyStock = async (req, res) => {
    try {
        ...
        const user = userRepository.findByEmail(getEmail(req.headers.authorization));

        const sell = sellRepository.findByPriceOrderDate(stock, price);

        if (sell === null) {
            const buyOrder = buyRepository.save({
                price: price,
                buyer_id: user.id,
                stock_id: stock}
            )
